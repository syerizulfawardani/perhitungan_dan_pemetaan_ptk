        ...
        <div class="card border-0 shadow-sm mb-4 overflow-hidden">
            ...
            <div class="card-body">
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <h6 class="fw-semibold text-uppercase text-muted mb-3" style="font-size:.7rem;letter-spacing:.08em">
                            <i class="ti ti-users me-1"></i> Data PTK Sekolah
                        </h6>
                        @if($s->dataPtk->isEmpty())
                            <div class="rounded-3 p-4 text-center" style="background:#f8f9fa">
                                ...
                                <p class="text-muted small mb-0 mt-2">Belum ada data PTK yang terdaftar<br>di sekolah ini.</p>
                            </div>
                        @else
                            <div class="d-flex flex-column gap-2">
                                @foreach($s->dataPtk->take(4) as $ptk)
                                <div class="d-flex align-items-center gap-2 p-2 rounded-2" style="background:#f8f9fa">
                                    <div class="rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                                         style="width:32px;height:32px;background:#e3eaff;font-size:.75rem;font-weight:700;color:#3d5af1">
                                        {{ strtoupper(substr($ptk->nama_ptk, 0, 1)) }}
                                    </div>
                                    <div class="flex-grow-1 overflow-hidden">
                                        <div class="fw-medium text-truncate" style="font-size:.875rem">{{ $ptk->nama_ptk }}</div>
                                        <div class="text-muted" style="font-size:.75rem">{{ $ptk->jabatan?->nama_jabatan ?? '-' }}</div>
                                    </div>
                                </div>
                                @endforeach
                                @if($s->dataPtk->count() > 4)
                                <div class="text-center">
                                    <small class="text-muted">+{{ $s->dataPtk->count() - 4 }} PTK lainnya</small>
                                </div>
                                @endif
                            </div>
                        @endif
